...
To allow these two use cases to be met, [ygot](https://github.com/lyckety/ygot) implements transformation of a YANG schema to a set of protobuf messages. The design choices made for this transformation are described [in this document](https://github.com/lyckety/ygot/blob/master/docs/yang-to-protobuf-transformations-spec.md).
...
The `ygot` package contains a `proto_generator` binary ([source](https://github.com/lyckety/ygot/tree/master/proto_generator)). This binary is used to generate the protocol buffer definitions. Rather than outputting an individual protobuf file with all message definitions within it, this binary has two modes of output:
...
import "github.com/lyckety/ygot/proto/ywrapper/ywrapper.proto";
import "github.com/lyckety/ygot/proto/yext/yext.proto";
...
This example walks through the generation of Protobuf files for the `openconfig-rib-bgp` model. Example code can be found in [`demo/protobuf_getting_started`](https://github.com/lyckety/ygot/tree/master/demo/protobuf_getting_started).
...
* A copy of the `ygot` `proto_generator` binary is required. Most simply, this can be installed through:
  * Retrieving ygot: `go get -u github.com/lyckety/ygot`
  * Installing ygot dependencies: `cd $GOPATH/src/github.com/lyckety/ygot && go get -t -d ./...`
...
```
go run $GOPATH/src/github.com/lyckety/ygot/proto_generator/protogenerator.go \
  -generate_fakeroot \
  -base_import_path="github.com/lyckety/ygot/demo/protobuf_getting_started/ribproto" \
  -go_package_base="github.com/lyckety/ygot/demo/protobuf_getting_started/ribproto" \
...
cd $GOPATH/src/github.com/lyckety/ygot/proto/yext && go generate
cd $GOPATH/src/github.com/lyckety/ygot/proto/ywrapper && go generate
```
...
find $GOPATH/src/github.com/lyckety/ygot/demo/protobuf_getting_started/ribproto -name "*.proto" | while read l; do
